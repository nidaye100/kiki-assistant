...
### 第四步：配置 Minecraft 服务器（可选）（无其他配置需要，应用路径全自动查找）
...
唤醒词使用 [openWakeWord](https://github.com/dscripka/openWakeWord) 实现，完全本地运行，无需注册任何账号。

默认使用内置的 `hey_jarvis` 模型。如需更换，修改 `core/wake_word.py` 顶部：

```python
WAKE_WORD_MODEL = "hey_jarvis"   # 可换为其他内置模型
THRESHOLD = 0.5                  # 灵敏度：越低越灵敏，越高越严格
```
...
如需训练自定义"Kiki"唤醒词，参考：https://github.com/dscripka/openWakeWord#training-new-models
